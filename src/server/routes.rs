...
pub fn router(state: AppState) -> Router {
    Router::new()
        .route("/metadata/{sha256}", get(get_metadata).put(put_metadata))
        .with_state(state)
}
...
/// transform, and stores + returns the resulting Field-schema metadata.
async fn put_metadata(
    ...
    state
        .metadata_store
        .save(actual.as_str(), &metadata)
        .map_err(ApiError::Store)?;

    Ok(Json(metadata))
}

/// Sink: retrieves a previously-extracted result without re-running GROBID.
